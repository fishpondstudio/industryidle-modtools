          ...
          return (
            <tr>
              ...
              <td class="nowrap text-right">
                <button onClick={() => this.banChat(hasChat[k].ip, 0)}>
                  0m
                </button>{" "}
                <button
                  onClick={() => this.banChat(hasChat[k].ip, 5 * 60 * 1000)}
                >
                  5m
                </button>{" "}
                <button
                  onClick={() => this.banChat(hasChat[k].ip, 30 * 60 * 1000)}
                >
                  30m
                </button>{" "}
                <button
                  onClick={() =>
                    this.banChat(hasChat[k].ip, 24 * 60 * 60 * 1000)
                  }
                >
                  24h
                </button>
                <div class="red bold">
                  {banChat > 0
                    ? `${
                        Math.round((banChat - Date.now()) / 100 / 60) / 10
                      }m Left`
                    : ""}
                </div>
              </td>
