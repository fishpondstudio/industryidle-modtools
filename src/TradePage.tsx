...
export class TradePage extends Component<{}, { trades: any; ban: any }> {
  ...
  async deleteTrade(userId: string) {
    if (window.confirm("Are you sure to delete this trade?")) {
      await fetch(
        `${API_HOST}/trade?token=${getUrlParams()?.token}&userId=${userId}`
      );
      await this.loadData();
    }
  }
        ...
        {Object.keys(this.state.trades).map((k) => {
          ...
          return (
            <tr>
              ...
              <td
                className={
                  trade.amount > 1e12 ? "red text-right" : "text-right"
                }
              >
              ...
              <td className="text-right">
                {Math.round((Date.now() - trade.createdAt) / 100 / 60) / 10}m
                ago
              </td>
              <td className="nowrap text-right">
                <button
                  onClick={() =>
                    navigator.clipboard.writeText(trade.fromUserId)
                  }
                  title={trade.fromIp}
                >
                  Copy
                </button>{" "}
                <button onClick={() => this.banTrande(trade.fromIp, 0)}>
                  0
                </button>{" "}
                <button
                  onClick={() =>
                    this.banTrande(trade.fromIp, 24 * 60 * 60 * 1000)
                  }
                >
                  1d
                </button>{" "}
                <button
                  onClick={() =>
                    this.banTrande(trade.fromIp, 7 * 24 * 60 * 60 * 1000)
                  }
                >
                  1w
                </button>{" "}
                <button
                  onClick={() =>
                    this.banTrande(trade.fromIp, 30 * 24 * 60 * 60 * 1000)
                  }
                >
                  1mo
                </button>{" "}
                <button onClick={() => this.deleteTrade(trade.fromUserId)}>
                  Delete
                </button>
