...
export class UserPage extends Page<{
   ...
   override async componentDidMount() {
      ...
      if (user?.platformId) {
         fetch(
            `${API_HOST}/platform-ban?token=${getUrlParams()?.token}&platformId=${encodeURIComponent(user.platformId)}`
         )
            .then((r) => r.json())
            .then((v) => {
            ...
            {this.state.user ? (
               <div class="buttons">
                  ...
                  <button
                     onClick={() => {
                        window.open(
                           `${API_HOST}/all-platform-id?token=${getUrlParams()?.token}&id=${this.props.params.id}`,
                           "_blank"
                        );
                     }}
                  >
   ...
   private platformIdBan(ban: boolean) {
      fetch(
         ...
         )}&comment=${encodeURIComponent(`${ban ? "!" : "~"}${this.state.user.userName}:${this.state.user.lastIp}`)}`,
         { method: "post" }
