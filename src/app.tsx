...
const API_HOST = "api.fishpondstudio.com";
...
export class App extends Component<{}, { chats: any; trades: any }> {
    ...
    loadData = () => {
        fetch(`https://${API_HOST}/stat?token=${getUrlParams()?.token}`)
            .then((r) => r.json())
            .then((j) => {
                this.setState({ chats: j });
            });
        fetch(`https://${API_HOST}/trade?token=${getUrlParams()?.token}`)
            .then((r) => r.json())
            .then((j) => {
